...
class _PostScreenState extends State<PostScreen> {
  ...
  void _fetchboard() async {
    ...
    final response = await http
        .get(Uri.parse('http://localhost:443/post/board?board_id=$board_id'));
  ...
  void _fetchintroduction() async {
    ...
    final response = await http
        .get(Uri.parse('http://localhost:443/user/info?student_id=$student_id'));
    if (response.statusCode == 201) {
      final responseData = jsonDecode(response.body);
  ...
  Future<List<dynamic>> fetchComments() async {
    final response = await http.get(Uri.parse(
        'http://localhost:443/post/comment:?post_id=${widget.post['post_id']}'));
  ...
  Future<void> postComment(int postId, String content) async {
    final url = Uri.parse('http://localhost:443/post/commentwrite/${widget.post['post_id']}');
  ...
  Future<void> _deleteComment(int commentId) async {
    final url = Uri.parse('http://localhost:443/post/deletecomment/$commentId');
    final storage = FlutterSecureStorage();
    final token = await storage.read(key: 'token');
  ...
  Future<void> _editComment(int commentId, String content) async {
    final url = Uri.parse('http://localhost:443/post/updatecomment/$commentId');
    final storage = FlutterSecureStorage();
    final token = await storage.read(key: 'token');
    if (token == null) {
      setState(() {
...
//댓글 수정 폼
  ...
  Future<void> deletePost() async {
    ...
    final response = await http.post(
      Uri.parse('http://localhost:443/post/deletepost/${widget.post['post_id']}'),
      headers: <String, String>{ //헤더파일 추가
        'Content-Type': 'application/json; charset=UTF-8',
        'Authorization': token,
      },
  ...
  Future<void> reportPost() async{
    final response = await http.post(
      Uri.parse('http://localhost:443/post/reportPost/${widget.post['post_id']}'),
    );
    if (response.statusCode == 200) {
  ...
  Future<void> _fetchProfile(String studentId) async {
    final response = await http.get(
      Uri.parse('http://localhost:443/user/info?student_id=$studentId'),
    );
    if (response.statusCode == 201) {
      final responseData = jsonDecode(response.body);
      setState(() {
