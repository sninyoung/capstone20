...
class _GScoreApcCtState extends State<GScoreApcCt> {
  ...
  Future<void> _getWriterInfo() async {
    ...
    final response = await http.get(
      Uri.parse('http://localhost:443/gScore/writer?student_id=${widget.post['gsuser_id']}'),
  ...
  Future<void> _fetchGsInfo() async {
    if (activityTypes.isEmpty) {
      final typeResponse = await http.get(Uri.parse('http://localhost:443/gScore/getType'));
  ...
  Future<void> _fetchNamesAndScores(String selectedType) async {
    if (!activityNames.containsKey(selectedType)) {
      ...
      final infoResponse = await http.get(Uri.parse('http://localhost:443/gScore/getInfoByType/$encodedType'));
  ...
  Future<void> _getFileInfo() async {
    final response = await http.get(
      Uri.parse('http://localhost:443/gScore/fileInfo?postId=${widget.post['gspost_id']}'),
      headers: <String, String>{
        'Content-Type': 'application/json; charset=UTF-8',
      },
    );
  ...
  Future<String?> downloadFile() async {
    ...
    final response = await http.get(
      Uri.parse('http://localhost:443/gScore/download?reqPath=${Uri.encodeComponent(uploadedFilePath ?? '')}'),
      headers: <String, String>{
        'Content-Type': 'application/json; charset=UTF-8',
      },
    );
      ...
      while (retryCount < maxRetries) {
        try {
          final request = http.MultipartRequest(
            'POST',
            Uri.parse('http://localhost:443/gScore/upload'),
          );
    ...
    while (retryCount < maxRetries) {
      try {
        final response = await http.post(
          Uri.parse('http://localhost:443/gScore/fileToDB'),
          headers: <String, String>{
            'Content-Type': 'application/json; charset=UTF-8',
          },
      ...
      while (retryCount < maxRetries) {
        try {
          final response = await http.delete(
            Uri.parse(
                'http://localhost:443/gScore/deleteFile?reqPath=${Uri
  ...
  Future<void> updatePost() async {
    ...
    final response = await http.post(
      Uri.parse('http://localhost:443/gScore/update'),
      headers: <String, String>{
        'Content-Type': 'application/json; charset=UTF-8',
        'Authorization': token,
      },
  ...
  Future<void> deletePost() async {
    ...
    final url = Uri.parse('http://localhost:443/gScore/deletePost?postId=$postId');
